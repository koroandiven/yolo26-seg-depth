...
class MaskEdgeDepthLoss(nn.Module):
    """Mask edge-aware depth loss - encourages sharp depth discontinuities at object boundaries."""

    def __init__(self, edge_weight=1.0, smooth_weight=0.1):
        super().__init__()
        self.edge_weight = edge_weight
        self.smooth_weight = smooth_weight

    def _compute_mask_edge(self, masks, batch_idx, shape):
        """Compute binary edge map from instance masks.

        Supports two input formats:
        - [B, H, W]: instance index map (overlap format, one per image)
        - [N_instances, H, W]: binary instance masks with batch_idx mapping

        Args:
            masks: instance masks tensor
            batch_idx: [N_instances] mapping each instance to batch image index (can be None for index map format)
            shape: (B, C, H, W) target shape for output edge map

        Returns:
            [B, 1, H, W] edge map (float, 0 or 1)
        """
        B, _, H, W = shape
        device = masks.device
        ...
        if masks.ndim == 3 and masks.shape[0] == B:
            ...
            combined = (masks > 0).float().unsqueeze(1)  # [B, 1, H, W]
        elif masks.ndim == 3 and batch_idx is not None:
            # [N_instances, H, W] binary instance mask format
            combined = torch.zeros(B, H, W, device=device, dtype=mask_dtype)
            bidx = batch_idx.view(-1)
            for i in range(B):
                inst_i = masks[bidx == i]  # [N_i, H, W]
                if inst_i.numel() > 0:
                    combined[i] = inst_i.float().sum(dim=0).clamp(0, 1)
            combined = combined.unsqueeze(1)  # [B, 1, H, W]
        else:
            # Fallback: return empty edge map
            return torch.zeros(B, 1, H, W, device=device, dtype=mask_dtype)

        # Sobel kernels for edge detection (always float)
        ...
        edge_x = F.conv2d(combined, sobel_x, padding=1).abs()
        edge_y = F.conv2d(combined, sobel_y, padding=1).abs()
        edge = (edge_x + edge_y).squeeze(1)  # [B, H, W]
        edge = (edge > 0.5).float().unsqueeze(1)  # [B, 1, H, W]
        return edge

    def _compute_depth_grad_magnitude(self, depth):
        """Compute depth gradient magnitude using Sobel operators.
        ...
        grad_x = F.conv2d(depth, sobel_x, padding=1).abs()
        grad_y = F.conv2d(depth, sobel_y, padding=1).abs()
        return grad_x + grad_y

    def forward(self, pred_depth, masks=None, batch_idx=None):
        """Compute mask edge-aware depth loss.

        Args:
            pred_depth: [B, 1, H, W] predicted depth
            masks: instance masks (can be [B, H, W] index map or [N, H, W] binary masks)
            batch_idx: [N_instances] instance-to-image mapping (optional for index map format)
        ...
        edge = self._compute_mask_edge(masks, batch_idx, pred_depth.shape)
        # Edge map may be at mask resolution (e.g. 160x160) while depth is at full resolution (640x640)
        if edge.shape[-2:] != pred_depth.shape[-2:]:
            edge = F.interpolate(edge, size=pred_depth.shape[-2:], mode="nearest")
        depth_grad = self._compute_depth_grad_magnitude(pred_depth)

        # Smoothness: penalize large depth gradients outside mask edges
        smooth_loss = ((1.0 - edge) * depth_grad).mean()

        # Edge alignment: encourage large depth gradients at mask edges
        # exp(-k * grad) penalizes small gradients exponentially
        edge_align_loss = (edge * torch.exp(-depth_grad * 3.0)).mean()
...
class DepthSegmentationLoss(v8SegmentationLoss):
    ...
    def loss(self, preds, batch):
        ...
        When freeze_seg=True, segmentation loss is computed without gradients
        to prevent NYU semantic labels from corrupting COCO pretrained backbone.
        ...
        if self.freeze_seg:
            # Block segmentation gradients from flowing back to backbone/seg head
            with torch.no_grad():
